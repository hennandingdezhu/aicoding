<template>
  <div>
    ...
    <div class="page-header">
      <h2>{{ isEdit ? '查看出库单' : '新增出库单' }} <span class="mock-badge">模拟</span></h2>
    </div>
    <div class="form-card">
      <div class="form-group"><label>仓库</label><select v-model="form.warehouseId"><option value="1">主仓库</option></select></div>
      <div class="form-group"><label>客户</label><select v-model="form.customerId"><option value="1">客户A</option><option value="2">客户B</option></select></div>
...
const form = ref({ warehouseId:'', customerId:'', productId:'', quantity:0, remark:'' })
onMounted(async () => {
  if (route.params.id) {
    isEdit.value = true
    const res = await api.getOutboundOrderById(route.params.id)
    Object.assign(form.value, res.data)
  }
})
async function handleSubmit() {
  await api.createOutboundOrder(form.value)
