<template>
  <div>
    ...
    <div class="page-header">
      <h2>{{ isEdit ? '查看入库单' : '新增入库单' }} <span class="mock-badge">模拟</span></h2>
    </div>
    <div class="form-card">
...
const form = ref({ warehouseId:'', productId:'', quantity:0, remark:'' })
onMounted(async () => {
  if (route.params.id) {
    isEdit.value = true
    const res = await api.getInboundOrderById(route.params.id)
    Object.assign(form.value, res.data)
  }
})
async function handleSubmit() {
  await api.createInboundOrder(form.value)
  router.push('/inbound')
}
</script>