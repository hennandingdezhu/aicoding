<template>
  <div>
    ...
    <div class="page-header">
      <h2>客户管理 <span class="mock-badge">模拟</span></h2>
      <button class="btn btn-primary" @click="$router.push('/customers/create')">新增客户</button>
    </div>
    <div class="table-card">
      <div class="search-bar">
        <input v-model="keyword" placeholder="搜索客户名称..." />
      ...
      <table>
        ...
        <tbody>
          <tr v-for="item in list" :key="item.id">
            ...
            <td>
              <button class="btn btn-default btn-sm" @click="$router.push('/customers/'+item.id+'/edit')">编辑</button>
...
async function fetchData() {
  const res = await api.getCustomers({ page:1, pageSize:20, keyword:keyword.value })
  list.value = res.data.records; total.value = res.data.total
}
async function handleDelete(id) { if (confirm('确定删除？')) { await api.deleteCustomer(id); fetchData() } }
onMounted(fetchData)
</script>