<template>
  <div>
    ...
    <div class="page-header">
      <h2>供应商管理 <span class="mock-badge">模拟</span></h2>
      <button class="btn btn-primary" @click="$router.push('/suppliers/create')">新增供应商</button>
    </div>
    <div class="table-card">
      <div class="search-bar">
        <input v-model="keyword" placeholder="搜索供应商名称..." />
      ...
      <table>
        ...
        <tbody>
          <tr v-for="item in list" :key="item.id">
            ...
            <td>
              <button class="btn btn-default btn-sm" @click="$router.push('/suppliers/'+item.id+'/edit')">编辑</button>
...
async function fetchData() {
  const res = await api.getSuppliers({ page:1, pageSize:20, keyword:keyword.value })
  list.value = res.data.records; total.value = res.data.total
}
async function handleDelete(id) { if (confirm('确定删除？')) { await api.deleteSupplier(id); fetchData() } }
onMounted(fetchData)
</script>